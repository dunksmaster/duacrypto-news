...
export function globalSecurityHeadersBlock() {
  ...
  const cspValue = [
    ...
    "connect-src 'self' https://www.google-analytics.com https://region1.google-analytics.com https://formspree.io https://getalby.com https://api.alby.io;",
    "frame-src https://www.google.com https://acceptable.a-ads.com https://a-ads.com;",
...
export function staticCacheHeadersBlocks() {
  ...
  return ["/img/*", "/_astro/*", "/pagefind/*", "/js/*", "/og/*"]
    .map((path) => `${path}\n${cache}`)
    .join("\n\n");
}

/** Allow iframe embeds on /embed/* routes only (CSP frame-ancestors *). */
export function embedHeadersBlock() {
  const embedCsp =
    "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; frame-ancestors *;";
  return ["/embed/*", "/en/embed/*"]
